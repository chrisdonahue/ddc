if __name__ == '__main__':
  ...
  glob_wildcard = (['*'] * args.depth) + ['*.raw.json']
    ...
    for depth_dir in depth_dirs:
      for split_name in split_names:
        ...
        with open(os.path.join(depth_dir, '{}.txt'.format(split_name)), 'w') as f:
          f.write('\n'.join(sorted(split_aggregated)))


  depth_dirs = set(pack_dirs)
  for _ in xrange(args.depth + 1):
    for depth_dir in depth_dirs:
      txt_fps = glob.glob(os.path.join(depth_dir, '*.txt'))
      all_fps = []
      for txt_fp in txt_fps:
        with open(txt_fp, 'r') as f:
          all_fps.extend([l.strip() for l in f.read().splitlines()])
      with open(os.path.join(depth_dir, 'all.txt'), 'w') as f:
        f.write('\n'.join(sorted(all_fps)))
